...
CODESTARTnewParserInst
	DBGPRINTF("newParserInst (pmrfc3164sd)\n");
	...
	if(Debug) {
		dbgprintf("parser param blk in pmrfc3164sd:\n");
	...
	for(i = 0 ; i < parserpblk.nParams ; ++i) {
		...
		if(!strcmp(parserpblk.descr[i].name, "detect.yearaftertimestamp")) {
			...
			dbgprintf("pmrfc3164sd: program error, non-handled "
...
CODESTARTfreeParserInst
	dbgprintf("pmrfc3164sd: free parser instance %p\n", pInst);
ENDfreeParserInst
